...
public class RobotContainer {
  ...
  /*Auto Builder needs the named commands to be registered before the creation of any Planner autos or paths */
    ...
    NamedCommands.registerCommand("RunIndex", new RunIndexer(s_Indexer,Constants.IndexerConstants.IndexVeloFWD,Constants.IndexerConstants.indexVeloAux));
    NamedCommands.registerCommand("StopShooter", new StopShooter(s_Shooter,s_Indexer));
    NamedCommands.registerCommand("StopIndex",new RunIndexer(s_Indexer, 0,0));
    NamedCommands.registerCommand("RunIndexRev",new RunIndexer(s_Indexer, Constants.IndexerConstants.indexVeloREV, Constants.IndexerConstants.indexVeloAux));
    NamedCommands.registerCommand("IntakeDeploy", new IntakeDeploy(s_Climber,-0.5,0.5).withTimeout(.5));
    NamedCommands.registerCommand("IntakeUnDeploy", new IntakeDeploy(s_Climber,0.5,-0.5).withTimeout(.55));
    NamedCommands.registerCommand("IntakeStopDeploy", new IntakeDeploy(s_Climber,0.0,0.0).withTimeout(1));
    ...
    private void configureButtonBindings() {
        ...
        runIndex.whileTrue(new RunIndexer(s_Indexer, Constants.IndexerConstants.indexVelo, Constants.IndexerConstants.indexVeloAux));
        runIndex.whileFalse(new RunIndexer(s_Indexer, 0,0));
